        ...
        for hover_sel in [
            ...
            try:
                ...
                if el:
                    ...
                    self.log.info("Hovered game container: %s", hover_sel)
                    break
            except Exception:
                continue
        ...
        for sel in [
            ...
            try:
                ...
                if el and await el.is_visible():
                    await el.click()
                    await asyncio.sleep(1.2)
                    self.log.info("Page-level Demo clicked via: %s", sel)
                    return True
            except Exception:
                continue
        self.log.info("Page-level Demo button not found — will try in-frame fallback.")
        return False
